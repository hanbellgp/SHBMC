...
namespace cn.hanbell.mcloud.HZCW017
{
    public class HZCW017
    {
        ...
        public string GetBasicSetting(XDocument pM2Pxml)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(entityJSONStr))//沒單據資料
                {
                    ...
                    if (!string.IsNullOrEmpty(tErrorMsg)) return Utility.ReturnErrorMsg(pM2Pxml, ref tP2MObject, tErrorMsg);
                    //回傳的JSON轉Class
                    BudgetCenter budgetCenter = Utility.JSONDeserialize<BudgetCenter>(tResponse);  //预算中心对象
                    string tcenterid = budgetCenter.centerid;
                    #endregion

                    #region 給值
                    //設定借支單資料
                    Entity entityClass = new Entity(tCompany, tDate, tUser, tDetp, "0", "销售营业");
                ...
                tP2MObject.AddRCPControls(tRCPcompany, tRCPapplyDate, tRCPuserid, tRCPdeptno, tRCPloanproperty, tRCPtrafficfee,
                    tRCPaccommodation, tRCPallowance, tRCPentertain, tRCPother, tRCPcoin,tRCPloanTotal,tRCPpreAccno,tRCPprePayDate,tRCPcenterid,tRCPdocData);
                #endregion
            }
            catch (Exception err)
            {
        ...
        public string GetPreAccno_OpenQuery(XDocument pM2Pxml)
        {
            ...
            try
            {
                ...
                //取得币別Table
                //DataTable tCoinTable = CoinClass.GetDataTable(tSearch);
                DataTable tpreAccnoTable = preAccnoClass.GetDataTable(tSearch, new string[] { "budgetacc", "accname" });
                tpreAccnoTable.Columns.Add("preAccnoC");//    Control applyUser + C = 開窗控件要顯示的值
                for (int i = 0; i < tpreAccnoTable.Rows.Count; i++) { tpreAccnoTable.Rows[i]["preAccnoC"] = tpreAccnoTable.Rows[i]["budgetacc"] + "-" + tpreAccnoTable.Rows[i]["accname"]; }//設定顯示的值
                ...
                #region 設定物件屬性
                //币別清單屬性
                tRDpreAccno.AddCustomTag("DisplayColumn", "accno§accname§preAccnoC");
                tRDpreAccno.AddCustomTag("StructureStyle", "accno:R1:C1:1§accname:R1:C2:1");
        ...
        //费用变更 核算金额
        public void GetTrafficfee_OnBlur(XDocument pM2Pxml) {
            //string tP2Mxml = string.Empty;//回傳值
            //ProductToMCloudBuilder tP2MObject = new ProductToMCloudBuilder(null, null, null); //參數:要顯示的訊息、結果、Title//建構p2m
            //#region 設定控件
            //RDControl tRDloanTotal = new RDControl(new DataTable());  //预算科目清單
            //#endregion
            ...
            string tEntertain = DataTransferManager.GetDataValue(pM2Pxml, "entertain");        //招待费
            ...
            try
            {
               tLoanTotal = (int.Parse(tTrafficfee) + int.Parse(tAccommodation) + int.Parse(tAllowance) + int.Parse(tOther)+ int.Parse(tEntertain)).ToString();
                //處理回傳
                //tP2MObject.AddRDControl(tRDloanTotal);
            }
            catch (Exception err) {
                throw err;
            }
    ...
    class Entity
    {
        ...
        public Entity(string company, string date, string id, string deptno, string loanproperty, string loanpropertyDesc)
        {
            ...
            this.loanproperty = loanpropertyDesc;
        }
